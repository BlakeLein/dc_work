...
from plurnSettings import Colors as c
...
************************************************************
    || NAME: {self.player.name} |||| RANK: {self.player.rank} ||''')
        print(f'''
    {c.BLUE}<<Hit Points>>{c.END}
        {self.player.currentHp} / {self.player.maxHp}''')
        print(f'''
    {c.BLUE}<<Attack Power>>{c.END}
        {self.player.power}''')
        print(f'''
    {c.BLUE}<<Items>>{c.END}''')
...
''')
        if rank == '1':
            self.player = Character(self.player.name, 'P.L.U.R.N. Corps Soldier', 6, 1, 22, 3)

        elif rank == '2':
            self.player = Character(self.player.name, 'P.L.U.R.N. Corps Operative', 6, 1, 15, 10)

        elif rank == '3':
            self.player = Character(self.player.name, 'P.L.U.R.N. Corps Medic', 4, 1, 25, 10)
